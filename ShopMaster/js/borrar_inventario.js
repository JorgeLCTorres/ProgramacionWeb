...
function confirmar_borrar(id_producto) {
    Toast.fire().then((result) => {
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                url: "../controller/crtl-borrar_inventario.php?id_producto=" + id_producto,
                // SI LA SOLICITUD SE REALIZA CORRECTAMENTE ENTONCES SE MUESTRA UN MODAL SWEETALERT
                success: function (r) {
                    Swal.fire("Eliminado con éxito", "El producto ha sido eliminado", "success").then((res) => {
                        // SI SE CONFIRMA EL MODAL DE ELIMINACIÓN EXITOSA ENTONCES SE RECARGA LA PÁGINA
                        if (res.isConfirmed) {
                            window.location.href = '../view/inventario.php';
                        }
                    });
                }
            });
